...
def func_open():
    global window,canvas,paper,photo,photo2,oriX,oriY
    readFp=askopenfilename(parent=window, defaultextension=".*", filetypes=(("모든 그림 파일","*.jpg;*.jpeg;*.bmp;*.png;*.tif;*.gif"),("모든파일","*.*") ))
...
def func_save():
    ...
    if photo2 == None :
        return
    saveFp = asksaveasfile(parent=window, mode="w", defaultextension=".*", filetypes=(("모든 그림 파일","*.jpg;*.jpeg;*.bmp;*.png;*.tif;*.gif"),("모든 파일","*.*"))) # 저장파일 입력
