...
def writeBoomMessage(str1): ##우주선이 우주괴물에 닿으면 메시지를 띄우고 게임 종료하는 함수
    ...
    monitor.blit(txt,(swidth/2-60, sheight/2-10))
    

def playGame() :  # 게임을 작동하는 본체함수
    global monitor,ship,monster,missile,fireCount,shipimage,count
    ...
    if a==1:
        ...
        monsterX=0
        monsterY=random.randrange(0,int(sheight * 0.7))
    ...
    while True : # 무한 반복 / 나중에 우주선 움직임과 미사일 발사되는 듯한 효과 볼수 있음
        ...
        if a==1:
            ...
            monsterY +=random.randrange(-monsterSpeed,+monsterSpeed)
            if monsterY+monsterSize[1] > sheight*0.7 :
