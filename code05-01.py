...
if __name__ == "__main__" :
    ...
    t3 = turtle.Turtle("turtle"); t2.color('blue'); t3.penup()
    ...
    while True :
        ...
        ##거북이끼리 서로 만나면 도장을찍는 코드##
        if math.sqrt(((t1X-t2X)*(t1X-t2X))+((t1Y-t2Y)*(t1Y-t2Y))) <= 20 : t1.stamp(); t2.stamp
        elif math.sqrt(((t1X-t3X)*(t1X-t3X))+((t1Y-t3Y)*(t1Y-t3Y))) <= 20 : t1.stamp(); t3.stamp()
        elif math.sqrt(((t2X-t3X)*(t2X-t3X))+((t2Y-t3Y)*(t2Y-t3Y)))<= 20: t2.stamp(); t3.stamp()

        ##거북이가 화면 밖으로 나가면 처음위치로 돌아오는 코드##
