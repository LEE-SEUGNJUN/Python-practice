import turtle; import random ;import sqlite3;
##전역 변수 선언 부분##
...
while True: ##짧은 선분 마다 색깔,각도,전진길이 등등 초기화시켜주기
    ...
    if (-swidth/2 <= curX and curX <=swidth/2) and(-sheight/2 <=curY and curY <= sheight/2) :
       ...
       num2 = round(turtle.ycor()) ##반올림
        ...
        if exitCount >=5: ##5번 밖으로 나가면 DB연결을 저장하고 종료 그리기종료
            con.commit()
            break
...
sql = "SELECT * FROM turtleTable WHERE 선분ID = 5" ##DB에서 선분ID가 5인ROW만 조회한단 문장을 sql변수에저장
